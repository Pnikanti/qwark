...
- **Each volume bar is as wide as its own neighbourhood allows.** Bars are the right mark — volume is a discrete amount at a point in time, and a filled area would claim work during a month off — but a bar has width and a time axis has no slots to put one in. Sizing every bar from the *average* gap merged them into a single block wherever two sessions fell on consecutive days, which reads as a rendering fault rather than as two sessions. Sizing them all from the *tightest* gap turned the whole panel into hairlines over one close pair. So the width is per bar: a cluster narrows and the rest stays readable. Sessions hours apart on a three-month axis share a pixel column, which is what being hours apart on a three-month axis looks like.
